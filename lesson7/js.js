...
const config = {
    ...
    /**
     * @returns Отдает парметр по которому мы ищем поле для выставления счета.
     */
    getScore() {
        return this.settings.scoreSelector;
    },
...
const map = {
    ...
    render(snakePointsArray, foodPoint) {
        ...
        this.usedCells.push(foodCell); // тут была ошибка foodPoint
    },
};
...
const game = {
    ...
    init(userSettings) {
        ...
        this.score = document.querySelector(this.config.getScore());
    ...
    // рассчитываем первоначальный счет
    getStartScore() {
    ...
    // Отображаем счет
    gameScore() {
        this.score.innerHTML = this.getCurrentScore();
        return this.score;
    },
    ...
    // расчет текущего счета игры
    getCurrentScore() {
        //количество съеденных ячеек равно
        const curentScore = this.snake.getBody().length - 1;
        return curentScore;
    },
...
window.onload = config.init({scoreSelector: '#score'});
